...
public class LoanFile {
	/**
	 * 대출 상품 데이터 파일 경로 저장 대출 상품 데이터 저장된 리스트
	 */
	...
	public static void load() {
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				LoanFile.filelist.add(new ManageLoan(temp[0], Integer.parseInt(temp[1]), Float.parseFloat(temp[2]),
	...
	public static void save() {
		try {
			...
			for (ManageLoan acc : LoanFile.filelist) {
				String line = String.format("%s,%d,%f,%d,%d\r\n", acc.getloanName(), acc.getprincipal(), acc.getRate(),
						acc.getloanPeriod(), acc.getrPeriod());
				writer.write(line);
			}
			writer.close();
		} catch (Exception e) {
