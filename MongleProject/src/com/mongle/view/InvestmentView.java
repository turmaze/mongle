...
public static void printAsciiTable(ArrayList<Investment> data,String invest) { // 표에 반복해서 출력하는 메서드
   ...
   public static void printAsciiTable222(ArrayList<Investment> data,String invest, int total,int removeN) { // 표에 반복해서 출력하는 메서드
	   ...
	   int printNum = 1;
	    for (int i = 0; i < data.size(); i++) {
	        if (invest.equals(data.get(i).getRealTitle())) {
	            ...
	            System.out.printf("%22s|%-3d|%-14s\t|%-18s\t|%15s\t|%,15d원|\n", " ", printNum, data.get(i).getTitleDepo(),
	                    data.get(i).getBankDepo(), data.get(i).getPrice(), data.get(i).getAmount());
	            printNum++;
	        }
	        j++;
	               ...
	               for (BankAccount acc : BankAccount.list) {
	                  if (acc.getAccountNumber()
	                     ...
	                     if (acc.getDepositAmount() > totalPrice) {
	                       ...
	                       //System.out.printf("%22매수 후 잔액은 %,f원입니다.\n", " ", rest);
	                        MongleVisual.stopper();
	                        loop = false;
	                     }
