...
public class Depo {
	...
	private static List<InfoProduct> searchDeposit(Scanner scan, List<InfoProduct> table) {
		...
		try {
			...
			JSONObject res = (JSONObject) root.get("result"); //루트

			JSONArray list = (JSONArray) res.get("baseList"); // 밑
			JSONArray option = (JSONArray) res.get("optionList"); // 밑밑
